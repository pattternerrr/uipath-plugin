...
| **`uip` CLI** | build, publish, run, analyze (validation), package inspect | Bash ile `uip rpa ...` |
| **UiPath resmi skill'leri** (`uip skills install --agent claude`) | rpa-workflow-architect, coded-workflow-architect rehberleri | otomatik tetiklenir |
...
Validation/build/run/install MCP'de **YOK** — `uip` CLI ile Bash'ten yap (örn. `uip rpa analyze --project-path . --output json`).
...
`Edit`/`Write` ile .xaml dosyasına dokunmak YASAK. Her XAML değişikliği `edit_workflow` ile. Etrafından dolaşmak da yasak — MCP çalışmıyorsa önce MCP'yi düzelt.

### 4. DOĞRULAMA ZORUNLU
Her değişiklikten sonra `uip rpa analyze` çalıştır (Bash). Studio açıksa fill_activity'nin bridge'i anlık state verir.
...
```
mcp_orientation → set_project_root → get_activity_metadata → fill_activity → edit_workflow → (Bash) uip rpa analyze
```
